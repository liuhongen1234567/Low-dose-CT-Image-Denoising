...
from utls.loss import Loss, GetMask
from utls.eval import test
from models.OurModel.our_model import OurModel
...
def main():
    ...
    model.to(device)
    save_dir ='pths'
    load_path ='{}/model_pretrained80.pth'.format(save_dir)
    save_path = '{}/our_model_save.pth'.format(save_dir)
    if os.path.exists(load_path):
        print("loading Network")
        model.load_state_dict(torch.load(load_path))
    opt = torch.optim.Adam(model.parameters(), betas=(0.9, 0.999), lr=lr)
    num_epoch = 160
    scheduler = lr_scheduler.MultiStepLR(opt,milestones=[130],gamma=0.5)
    ...
    try:
        for epoch in range(num_epoch):
            ...
            for i, (img, gt) in enumerate(train_loader):
                ...
                predict1 = model(img)
                step1_loss, MSE_Loss1, MS_SSIM_Loss1, Gradient_loss1 = Loss(predict1, gt)
                ...
                if Mask_Enable:
                    ...
                    step2_loss, MSE_Loss2, MS_SSIM_Loss2, Gradient_loss2 = Loss(predict2, filter_gt)
                    total_loss = step1_loss + step2_loss
                else:
                    step2_loss, MSE_Loss2, MS_SSIM_Loss2, Gradient_loss2 = 0,0,0,0
                ...
                if i % 50 == 0:
                    print(
                        "epoch {}/{} iter: {} Step 1: total loss {:.4f}, mse loss {:.4f}, MS-SSIM {:.4f}, Gradient Loss {:.4f}, Step 2: total loss {:.4f}, mse loss {:.4f}, MS-SSIM {:.4f}, Gradient Loss {:.4f}, total_loss {:.4f}, memory {:.4f} ".format(
                            epoch,num_epoch, i, step1_loss, MSE_Loss1,MS_SSIM_Loss1,Gradient_loss1,
                            step2_loss,MSE_Loss2,MS_SSIM_Loss2,Gradient_loss2,total_loss,
